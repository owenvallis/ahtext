...
public class MandalaNode implements TuioObserver {
	...
	public MandalaNode(StateMandala stateMandala, AhTextContext ahTextContext) {
		...
		textYAlign = ahTextContext.height / 20;
		pulser = (float) .015;
		...
		nodeDiameter = this.ahTextContext.height / (float) 10;
		nodeRadius = nodeDiameter / 2;
		nodeStrokeWeight = this.ahTextContext.height / 60;

		// node fade fields
		nodeFadeDiameter = this.ahTextContext.height / (float) 9;
		nodeFadeStrokeWeight = this.ahTextContext.height / 36;
		lineFadeStrokeWeight = this.ahTextContext.height / 30;
		fadeRate = 4;
	...
	boolean over(float nodePostionX, float nodePostionY, int x, int y) {
		...
		float disX = (nodePostionX + (stateMandala.xCenter * stateMandala.scaleFactor)) - x;
		float disY = (nodePostionY + (stateMandala.yCenter * stateMandala.scaleFactor)) - y;
	...
	private void resetNodeTouchTimer() {
		lastNodeTouchTime = ahTextContext.millis();
	}
	...
	public void drawMandalaNode() {
		ahTextContext.fill(255);
		ahTextContext.stroke(0);
		ahTextContext.strokeWeight(nodeStrokeWeight);
		ahTextContext.ellipseMode(PConstants.CENTER);
		ahTextContext.ellipse(nodeCenterX, nodeCenterY, nodeDiameter, nodeDiameter);
	}
	...
	public void fadeOverlay() {
		...
		drawFadeNode();
		drawTBarFades();
		if (ahTextContext.millis() - lastNodeTouchTime > nodeTouchTime) {
			triggerActive = true;
			if (!animationActive) {
				g2d.setColor(black);
				g2d.setFont(font);
				paintHorizontallyCenteredText(g2d, "TOUCH", nodeCenterX, nodeCenterY + (ahTextContext.textWidth("G") / 2));
			}
		}
	}
	...
	public void drawFadeNode() { // TODO figure out better way to extend to
									...
									// private
		ahTextContext.strokeWeight(nodeFadeStrokeWeight);
		defineFadeColor();
		ahTextContext.ellipse(nodeCenterX, nodeCenterY, nodeFadeDiameter, nodeFadeDiameter);
	}
	...
	private void drawTBarFades() {
		ahTextContext.strokeWeight(lineFadeStrokeWeight);
		ahTextContext.line(spokeXA, spokeYA, spokeXB, spokeYB);
		ahTextContext.line(tbarCounterClockwiseXA, tbarCounterClockwiseYA,
				tbarCounterClockwiseXB, tbarCounterClockwiseYB);
		ahTextContext.line(tbarXA, tbarYA, tbarXB, tbarYB);
	}
	...
	public void defineFadeColor() {
		ahTextContext.stroke(this.redTemp += (fadeRate * this.fadeStateRed),
				this.greenTemp += (fadeRate * this.fadeStateGreen),
				this.blueTemp += (fadeRate * this.fadeStateBlue),
				this.alphaTemp += (fadeRate * this.fadeStateAlpha)); // define the color
	}
	...
	public void setNumberOfNodes(int numberOfNodes) {
		if (numberOfNodes > 0)
			MandalaNode.numberOfNodes = numberOfNodes;
		else
			MandalaNode.numberOfNodes = 1;
	}
